...
#[post("/create-report")]
async fn create_report(body: web::Json<RequestBody>) -> impl Responder {
    println!("[create-report] body: {:#?}", body);
    let unique_folder_name = Uuid::new_v4().to_string();
    ...
    match &body.url {
        Some(url) => sitetopdf.arg("-u").arg(url),
        None => {
            let html = match &body.html {
                Some(html) => html,
                None => "",
            };
            let css = match &body.css {
                Some(css) => css,
                None => "",
            };
            let html_content = format!(
            ...
            sitetopdf.arg("-x").arg(html_content)
        }
    };

    let output_file = format!("{}/{}", &pdf_folder_path, &body.file_name);
    sitetopdf.arg("-o").arg(output_file).arg("-v");
